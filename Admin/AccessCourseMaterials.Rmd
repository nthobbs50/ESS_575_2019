...
[VI. Install course-specific R package ``ESS575``][] 
...
Open terminal/bash and navigate to the directory where you would like to hold the course materials. If you are not familiar or comfortable using the command line, I **highly** recommend using the root directory. For the rest of this quick intro, I will refer to the path to your root directory as ``rootPath``.  
...
2.  To clone the course repository into your current directory paste the following into your terminal/bash window: 
    ...
    ```bash
    git clone https://github.com/nthobbs50/ESS_575_2017.git
    ```
...
I will add and modify course material frequnetly. I urge you to update the course material on your computer shortly before lecture and lab -- 9:00 on Tuesday Thursday and noon on Wednesday. To do this you will ``pull`` a current copy of the repository. To do this: 
...
Each time you ``pull`` and updated version of the course repository your local machine will have a clean (untouched) copy of **all** course materials.  **Therefore, if you modify the course materials it is extremely important that you save your work under a different name.**  If you fail to save your work with a new name, your work will be lost the next time you ``pull`` to get the most up-to-date materials. Where you choose to save your work doesn't matter. You can choose a brand new directory or you can save within the directory that you've created for our course ``ESS_575_2017``. 

Inevitably, you will make an inadvertent or deliberate change to a file in the course materials stored in the `ESS_575_2017`  folder. In this case, your ``pull`` will fail.  To remedy this:
...
3.  Open R from the command line by type.
    ...
    R
    ```
4. Run the following line of code in R to install the ``ESS575`` package from source. You can also install it using the same command directly in RStudio but you will need to write the full path to the file or set the working directory to ``rootPath/ESS_575_2017``.

    ```bash
    install.packages("Admin/ESS575_0.1.0.tar.gz", repos = NULL, type = "source")
    ```
...
7.  To see the help files for ``ESS575`` go to the `Help` menu in Rstudio, choose `R Help`, click on `Packages` and follow your nose to ESS575.  (For some inexplicable reason, entering `?ESS575` at the console prompt doesn't work.  grrrrr.)
